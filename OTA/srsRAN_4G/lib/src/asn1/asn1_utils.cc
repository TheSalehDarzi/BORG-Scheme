...
namespace asn1 {
  ...
  //mirza:start
std::pair<std::unique_ptr<unsigned char[]>, size_t> extract_bytes_from_cbit_ref(const cbit_ref& bref) {
  auto byte_range = bref.get_byte_range();   
  const uint8_t* start = byte_range.first;
  size_t len = byte_range.second;

  std::unique_ptr<unsigned char[]> buffer(new unsigned char[len]);
  std::memcpy(buffer.get(), start, len);
  return {std::move(buffer), len};
}
//mirza:end
