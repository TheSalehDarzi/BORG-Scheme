...
//Header Files
#include "srsran/asn1/UE_T_BORG.h"
...
std::string tborg_basepath = "/home/masfiqur/5G_cryptobs/credentials/threshold_borg/";
...
void write_bn_to_file(const BIGNUM *bn, const char *filename) {
    FILE *fp = fopen((tborg_basepath+filename).c_str(), "w");
...
BIGNUM *read_bn_from_file(const char *filename) {
    ...
    FILE *fp = fopen((tborg_basepath+filename).c_str(), "r");
...
int verify_signature(const EC_GROUP *curve, const BIGNUM *order, BN_CTX *ctx, const EC_POINT *g, const EC_POINT *MPK,
    ...
    unsigned char *h_amf_input = (unsigned char*)malloc(id_amf_len + strlen(mpk_hex) + strlen(qamf_hex));
    ...
    unsigned char *h_bs_input = (unsigned char*)malloc(id_bs_len + strlen(qamf_hex2) + strlen(qid_hex2));
    ...
    unsigned char *h_input = (unsigned char*)malloc(h_input_len);
...
int run_tborg_UE(){
    ...
    //========================================= Load everything from files ===================================================
        ...
        const char* temp1 = "AMF-001";
        const char* temp2 = "BS-000001";
        unsigned char ID_AMF[7];
        unsigned char ID_BS[9];
        memcpy(ID_AMF,temp1,7);
        memcpy(ID_BS,temp2,9); 
