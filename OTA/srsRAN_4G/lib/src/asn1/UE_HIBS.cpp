...
// h_BS = H1(ID_BS || Q_AMF || QID_BS)
    ...
    unsigned char *concat2 = (unsigned char*)malloc(concat_len2 + 1);
...
int run_hibs_ue(){
    ...
    memcpy(message,temp,MSG_LEN);

    // // Read message
    // unsigned char message[MSG_LEN];
    // FILE *fmsg = fopen((hibs_basepath+"message.txt").c_str(), "rb");
    // if (!fmsg) {
    ...
    // Read MPK
    EC_POINT *MPK = read_point_from_file_hibs("MPK.txt", curve, ctx);
    print_point("MPK", curve, MPK, ctx);

    // Read Q_AMF
    EC_POINT *Q_AMF = read_point_from_file_hibs("Q_AMF.txt", curve, ctx);
    print_point("Q_AMF", curve, Q_AMF, ctx);

    // Read Q_BS
    EC_POINT *QID_BS = read_point_from_file_hibs("Q_BS.txt", curve, ctx);
    ...
    // Read R_j
    EC_POINT *R_j = read_point_from_file_hibs("R_j.txt", curve, ctx);
    print_point("R_j", curve, R_j, ctx);

    // Read z_j
    BIGNUM *z_j = read_bn_from_file_hibs("z_j.txt");
    print_bn("z_j", z_j);
