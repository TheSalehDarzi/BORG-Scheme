...
struct sib1_v1700_ies_s {
  ...
  bool                            sign_schnorr_hibs_present            = false;
  //mirza:end
  ...
  //mirza:start
  pqs_signature                   signature;
  fixed_bitstring<456>            Rj_bits; //114 hex digits, so 8*114/2 = 456
  fixed_bitstring<256>            hj_bits; //64 hex digits, so 8*64/2 = 256
  fixed_bitstring<224>            zj_bits; //56 hex digits, so 8*56/2 =224
  //mirza:end
  // sequence methods
  SRSASN_CODE pack(bit_ref& bref) const;
  SRSASN_CODE unpack(cbit_ref& bref);
