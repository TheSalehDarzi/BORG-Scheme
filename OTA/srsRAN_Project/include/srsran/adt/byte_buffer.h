...
//mirza:contains signing functionality
...
//mirza:start
// std::optional<byte_buffer> convert_to_byte_buffer(const byte_buffer_view& view) {
//   byte_buffer_memory_resource& pool = get_default_byte_buffer_segment_pool();
//   /// Creates an empty byte_buffer.
//   auto buffer = byte_buffer::create(pool);
//   if (!buffer) {
//       return std::nullopt; // Failed to create buffer
//   }

//   if (!buffer->append(view)) {
//       return std::nullopt; // Failed to append data
//   }
...
public:
  ...
  //mirza:start
  // Constructor from byte_buffer_view
  static std::optional<byte_buffer> from_view(const byte_buffer_view& view) {
    byte_buffer_memory_resource& pool = get_default_byte_buffer_segment_pool();
    
    auto buffer_expected = create(pool);
    if (!buffer_expected) {
        return std::nullopt;
    }

    byte_buffer buffer = std::move(buffer_expected.value());
    if (!buffer.append(view)) {
        return std::nullopt;
    }
  ...
  static std::vector<unsigned char> byte_buffer_to_char(const byte_buffer& buffer) {
    if (buffer.empty()) {
        return {};
    }
    // Allocating a new buffer to store the contiguous data
    std::vector<unsigned char> char_buffer(buffer.length()); // +1 if we want to account for null terminator
  
    size_t idx = 0;
    for(const auto& segment:buffer.segments()){
      std::memcpy(char_buffer.data() + idx, segment.data(), segment.size());
      idx += segment.size();
    }
    fmt::print("\n--------------Size of the message {}-----------------\n",char_buffer.size());
    //char_buffer[buffer.length()] = '\0';
    //printf("The vector buffer is %s",char_buffer);
    return char_buffer;
  }
  //mirza:end
