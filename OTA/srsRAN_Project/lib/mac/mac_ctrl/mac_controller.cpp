...
//mirza:start
void mac_controller::update_bcch_payload(du_cell_index_t cell_index, const byte_buffer& new_sib1_payload){
  auto* cell_ctrl_ptr = dynamic_cast<mac_cell_processor*>(&get_cell_controller(to_du_cell_index(cell_index)));
  srsran_assert(cell_ctrl_ptr != nullptr, "Invalid cell controller type");

  auto& sib_assembler = cell_ctrl_ptr->get_sib_pdu_assembler(); // add this accessor if not yet exposed

  unsigned new_version = sib_assembler.handle_new_sib1_payload(new_sib1_payload);

  srslog::fetch_basic_logger("MAC").info("Updated SIB1 for cell={} with version={}", unsigned(cell_index), new_version);
}
//mirza:end
