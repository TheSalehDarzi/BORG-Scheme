                                           ...
                                           span<const units::bytes>      si_payload_sizes)
{
  ...
  fmt::print("du cfg has si config set = {}\n",du_cfg.si_config.has_value());
  srsran_assert(si_payload_sizes.size() - 1 ==
                    (du_cfg.si_config.has_value() ? du_cfg.si_config->si_sched_info.size() : 0), //mirza: setting the right item of ternary condition from 0 to high
