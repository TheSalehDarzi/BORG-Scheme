...
std::string hibs_basepath = "/home/masfiqur/5G_cryptobs/credentials/hibs_schnorr/";
...
void write_bn_to_file_hibs(const BIGNUM *bn, const char *filename) {
    FILE *fp = fopen((hibs_basepath+filename).c_str(), "w");
...
BIGNUM *read_bn_from_file_hibs(const char *filename) {
    ...
    FILE *fp = fopen((hibs_basepath+filename).c_str(), "r");
...
void system_setup_HIBS(EC_GROUP *curve, EC_POINT **PK0, BIGNUM **sk0, BIGNUM *order, BN_CTX *ctx, unsigned char *hash, double *system_setup_time) {
    ...
    *sk0 = BN_dup(alpha);
    ...
    print_bn("msk", *sk0);
    write_bn_to_file_hibs(*sk0, "msk.txt");
    ...
    *PK0 = EC_POINT_new(curve);
    ...
    double system_setup_time2 = elapsed_time_get_alpha0 + elapsed_time_compute_MPK;
    ...
    * system_setup_time = system_setup_time2;
    ...
    printf("MPK: %s\n", pk0_hex);
    write_point_to_file_hibs(curve, *PK0, "MPK.txt", ctx);
...
void key_extract_for_AMF_HIBS(const EC_GROUP *curve, const EC_POINT *MPK, const BIGNUM *msk, const BIGNUM *order, BN_CTX *ctx,
    ...
    *Q_AMF = EC_POINT_new(curve);
    ...
    EC_POINT_mul(curve, *Q_AMF, alpha1, NULL, NULL, ctx);
    ...
    unsigned char *concat = (unsigned char*)malloc(concat_len + 1);
    ...
    *sk_AMF = BN_new();
    ...
    BN_mod_add(*sk_AMF, *sk_AMF, alpha1, order, ctx);
    ...
    write_bn_to_file_hibs(*sk_AMF, "sk_AMF.txt");
    write_point_to_file_hibs(curve, *Q_AMF, "Q_AMF.txt", ctx);

    double AMF_KeyExtraction_time2 = elapsed_time_sk_AMF + elapsed_time_hID1+ elapsed_time_compute_Q_AMF + elapsed_time_get_alpha1;
...
void key_extract_for_BSs(const EC_GROUP *curve, const EC_POINT *Q_AMF, const BIGNUM *sk_AMF, const BIGNUM *order, BN_CTX *ctx, BIGNUM **sk_BS_root_out, EC_POINT **QID_BS_out, BIGNUM **h_BS, double *BS_KeyExtraction_time) {
    ...
    *h_BS = BN_new();
    ...
    BN_mod_add(sk_BS_root, sk_BS_root, alpha_bs, order, ctx);
    ...
    *sk_BS_root_out = BN_dup(sk_BS_root);
    ...
    write_bn_to_file_hibs(sk_BS_root, "sk_BS.txt");

    double BS_KeyExtraction_time2 = elapsed_time_get_alpha_BS + elapsed_time_get_Q_BS + elapsed_time_H1_BS + elapsed_time_sk_BS;
...
int run_hibs_amf(){
    ...
    printf("\n========= RUN #%d =========\n", timing + 1);
        ...
        // Load the curve
        curve = EC_GROUP_new_by_curve_name(HIBS_CURVE_NAME);
        if (!curve) {
    ...
    //====================================== System Setup Algorithm ======================================    
        ...
        system_setup_HIBS(curve, &MPK, &msk, order, ctx, hash, &system_setup_time);
    ...
    //====================================== Key Extraction Algorithm for AMF ======================================
        ...
        key_extract_for_AMF_HIBS(curve, MPK, msk, order, ctx, &Q_AMF, &sk_AMF, &hID1_AMF, &AMF_KeyExtraction_time);
    ...
    //====================================== Key Extraction Algorithm for BSs ======================================
        ...
        key_extract_for_BSs(curve, Q_AMF, sk_AMF, order, ctx, &sk_BS_root, &QID_BS, &hID1_BS, &BS_KeyExtraction_time);
