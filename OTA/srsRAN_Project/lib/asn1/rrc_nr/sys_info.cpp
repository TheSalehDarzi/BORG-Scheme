...
{
  fmt::print("---------------------ACTUAL SIB PACK----------------------------\n");
  double raw_packet_start_time = get_time_ns();
  ...
  double raw_packet_end_time = get_time_ns();
  double raw_elapsed = (raw_packet_end_time - raw_packet_start_time)/1e9;
  //mirza:start
  if(sign_dilithium_present){
    FILE *fp = fopen("Dilithium_Printed_Output.txt", "w");
    if (!fp) {
    ...
    printf("\n--------- Certificate Signing ---------\n");
    sign_message(Global_CA_sk, CKG_pk, pk_len, CKG_cert, &CKG_cert_len);
    sign_message(CKG_sk, BS_pk, pk_len, BS_cert, &BS_cert_len);
    ...
    //     message[i] = rand() % 256;
    // }
    double timer1 = get_time_ns();
    std::vector<unsigned char> message;
    auto buffer_opt = srsran::byte_buffer::from_view(bref.data());
    if (buffer_opt) {  // Check if it contains a valid byte_buffer
      srsran::byte_buffer buffer = std::move(buffer_opt.value());
      message = srsran::byte_buffer::byte_buffer_to_char(buffer);
    }
    double timer2 = get_time_ns();

    printf("Message: %s", message.data());
    int message_length = message.size();
    printf("(size: %u bytes)", message_length);
    printf("\n");
    write_to_file(fp, "Message", message.data(), message_length);

    uint8_t signature[sig_len_max];
    size_t signature_len;
    sign_message(BS_sk, message.data(), message_length, signature, &signature_len);
    print_short_hex("Message Signature", signature, signature_len);
    write_to_file(fp, "Message Signature", signature, signature_len);

    fclose(fp);
    double dilit_packing_start_time = get_time_ns();
    signature_len = 72;
    BS_cert_len = 72;
    //Packing Signature
    for (size_t i = 0; i < signature_len; i++) {
      int intValue = signature[i];
      HANDLE_CODE(pack_integer(bref,intValue,0,255));
    }

    for (size_t i = 0; i < BS_cert_len; i++) {
      int intValue = BS_cert[i];
      HANDLE_CODE(pack_integer(bref,intValue,0,255));
    }
    double dilit_packing_end_time = get_time_ns();

    double elapsed_time_dilit_packing = (dilit_packing_end_time-dilit_packing_start_time) / 1e9;

    fmt::print("\nDilit Signature and Certificate Packing phase took {} seconds\n", elapsed_time_dilit_packing);

    fmt::print("\nTotal Packing phase took {} seconds\n", elapsed_time_dilit_packing+raw_elapsed+(timer2-timer1)/1e9);

    printf("\nFull cryptographic outputs saved to 'Printed_Output.txt'\n");
    printf("All steps completed successfully!\n");



  }
  else if(sign_hibfss_present){
    if(initialize_hibfss){
    ...
    int message_length = message.size();
    //for(int p=0;p<message_length;p++)fmt::print("This is {}\n",message.data()[p]);

    double sign_start_time = get_time_ns();
    
    do_sign_hibfss(message.data(), message_length);
    fmt::print("Things are okay\n");
    double sign_end_time = get_time_ns();
    double sign_packing_start_time = get_time_ns();
    BN_CTX *ctx = NULL;
    ctx = BN_CTX_new();
    if (!ctx) {
    ...
    // EC_POINT *Rj;
    fmt::print("Things are okay\n");
    std::string hj_hex = std::string(BN_bn2hex(hj));
    std::string zj_hex = std::string(BN_bn2hex(zj));
    std::string Rj_hex = std::string(EC_POINT_point2hex(curve, Rj, POINT_CONVERSION_UNCOMPRESSED, ctx));
    //std::string vec_QIDK_hex = std::string(EC_POINT_point2hex(curve, Rj, POINT_CONVERSION_UNCOMPRESSED, ctx));
    fmt::print("char Rj: {}\nchar hj: {}\nchar zj {}\n",Rj_hex.length(), hj_hex.length(), zj_hex.length());//114, 64, 56
    //Rj_bits = Rj_bits.from_string(Rj_hex);
    
    //Packing hj
    for(long unsigned int p = 0; p<hj_hex.length();p+=2){
      std::string curr_integer = hj_hex.substr(p, 2); // Extract two characters
      int intValue = std::stoi(curr_integer, nullptr, 16); // Convert hex to int
      //fmt::print("Slice {}: integer {}",p/2,intValue);
      HANDLE_CODE(pack_integer(bref,intValue,0,255));
    }

    //Packing Rj
    for(long unsigned int p = 0; p<Rj_hex.length();p+=2){
      std::string curr_integer = Rj_hex.substr(p, 2); // Extract two characters
      int intValue = std::stoi(curr_integer, nullptr, 16); // Convert hex to int
      //fmt::print("Slice {}: integer {}",p/2,intValue);
      HANDLE_CODE(pack_integer(bref,intValue,0,255));
    }
    ...
    //Packing zj
    for(long unsigned int p = 0; p<zj_hex.length();p+=2){
      std::string curr_integer = zj_hex.substr(p, 2); // Extract two characters
      int intValue = std::stoi(curr_integer, nullptr, 16); // Convert hex to int
      //fmt::print("Slice {}: integer {}",p/2,intValue);
      HANDLE_CODE(pack_integer(bref,intValue,0,255));
    }

    double sign_packing_end_time = get_time_ns();
    double elapsed_time_sign_packing = (sign_packing_end_time - sign_packing_start_time) / 1e9;

    //total 117 bytes need to be sent
    ...
    fmt::print("Message Signing phase took: {} seconds\n", elapsed_time_sign);
    fmt::print("Signature Packing phase took: {} seconds\n", elapsed_time_sign_packing);
    
    //HANDLE_CODE(signature.pack(bref)); //create a structure called signature, alt: can I directly do it inside bref buffer?
  }

  if(sign_tborg_present){
    fmt::print("TBOG signing starts\n");
    bool tborg_amf_required = false;
    if (tborg_amf_required)run_tborg_AMF();
    run_tborg_BS();
    fmt::print("TBORG signing ends\n");
  }


  if(sign_ec_schnorr_present){
    fmt::print("EC-Schnorr signing starts\n");
    bool ec_schnorr_amf_required = false;
    if (ec_schnorr_amf_required)run_ec_schnorr_amf();
    run_ec_schnorr_bs();
    fmt::print("EC-Schnorr signing ends\n");
  }

  if(sign_schnorr_hibs_present){
    fmt::print("Schnorr-HIBS Signing\n");
    bool hibs_amf_required = false;
    if (hibs_amf_required)run_hibs_amf();
    run_hibs_bs();
    fmt::print("Schnorr-HIBS signing ends\n");

  }

  if(sign_ecdsa_present){
    fmt::print("ECDSA Signing\n");
    bool ecdsa_amf_required = true; //keep it always true
    if (ecdsa_amf_required)run_ecdsa_amf();
    run_ecdsa_bs();
    fmt::print("ECDSA signing ends\n");
