 ...
 *
 *Compile:          gcc BORG.c -o BORG -lcrypto -lssl
 * 
 *Run:              ./BORG
 *
...
// std::string basepath = "/home/masfiqur/5G_cryptobs/credentials/threshold_borg/";
/*************************************************************
				    F u n c t i o n s
**************************************************************/
...
//     FILE *fp = fopen((basepath+filename).c_str(), "w");
...
//     FILE *fp = fopen((basepath+filename).c_str(), "r");
...
void read_commitment_scalars(int i, int j, BIGNUM **e_out, BIGNUM **d_out) {
    ...
    int ii, jj;
    FILE *fp = fopen((tborg_basepath+"ed1.txt").c_str(), "r");
...
{
    FILE *fp_bn = fopen((tborg_basepath+"ed1.txt").c_str(), "r");
    ...
    FILE *fp_pt = fopen((tborg_basepath+"ED2.txt").c_str(), "r");
...
void preprocess_commitments_for_BSs(int n, int J, const EC_GROUP *curve, const BIGNUM *order, EC_POINT *g, BN_CTX *ctx,
    ...
    FILE *fp_bn = fopen((tborg_basepath+"ed1.txt").c_str(), "w");
    FILE *fp_pt = fopen((tborg_basepath+"ED2.txt").c_str(), "w");
    if (!fp_bn || !fp_pt) {
    ...
    const EC_POINT *R_j, BIGNUM **h_j_out, BIGNUM **z_ij_out,const EC_POINT *QID_BS, double *time) 
{
    ...
    if (*h_j_out == NULL) {
        ...
        unsigned char *h_input = (unsigned char*)malloc(total_len);
...
int run_tborg_BS(){
    ...
    //========================================= Initialize OpenSSL objects ===================================================
        ...
        // Load the curve
        curve = EC_GROUP_new_by_curve_name(TBORG_CURVE_NAME);
        if (!curve) {
    ...
    //====================================== Preprocessing Algorithm ==========================================
        ...
        EC_POINT **E_vec = (EC_POINT**)malloc(n * J * sizeof(EC_POINT *));
        EC_POINT **D_vec = (EC_POINT**)malloc(n * J * sizeof(EC_POINT *));
        BIGNUM **e_scalars = (BIGNUM**)malloc(n * J * sizeof(BIGNUM *));
        BIGNUM **d_scalars = (BIGNUM**)malloc(n * J * sizeof(BIGNUM *));
    ...
    //====================================== Signing Algorithm ==========================================
        ...
        int *indices = (int*)malloc(t * sizeof(int));
        for (int x = 0; x < t; x++) indices[x] = x + 1;

        // Message to be signed
        const char* temp = "This is a 38-byte message for signing!";
        unsigned char message[38];
        memcpy(message, temp, 38);
        ...
        double *z_ij_time = (double*)malloc(t * sizeof(double));
